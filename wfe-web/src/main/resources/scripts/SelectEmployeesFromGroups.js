...
function dialogUNIQUENAME() {
	$.getJSON("JSON_URL", {
		tag : "SelectEmployeesFromGroups",
		qualifier : "UNIQUENAME",
		group : $("#groupFilterUNIQUENAME").val(),
		hint : $("#nameFilterUNIQUENAME").val(),
		t : new Date().getTime()
	}, function(data) {
		...
		$.each(data, function(i, item) {
			...
			var actorDiv = "<div id='divUNIQUENAME" + item.code + "' class='"
					+ divClass + "' title='" + divTitle + "'>";
			if (addEnabled) {
				actorDiv += "<a code='" + item.code
						+ "' href='javascript:addUNIQUENAME(" + '"' + item.code
						+ '", "' + item.name + '");' + "'>" + item.name
						+ "</a>";
			} else {
				actorDiv += item.name;
...
function addUNIQUENAME(code, name) {
	...
	e += "<input type='hidden' name='VARIABLENAME[" + rowIndex + "]' value='"
			+ code + "' /> ";
